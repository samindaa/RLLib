...
public:
  ...
  Simulator(Control<T, O>* agent, Environment<O>* environment, int nbRuns, int maxEpisodeTimeSteps,
      int nbEpisodes) :
  ...
  void step()
  {
    ...
    if (environment->getTRStep().endOfEpisode || (timeStep == maxEpisodeTimeSteps))
    {
      ...
      if (verbose)
      {
        ...
        std::cout << "{" << nbEpisodeDone << " ["<<timeStep << " (" << episodeR << "," << episodeZ << "," << averageTimePerStep
            << ")]} ";
        //std::cout << ".";
        std::cout.flush();
      }
