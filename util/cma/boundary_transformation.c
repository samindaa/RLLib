...
void boundary_transformation_init(boundary_transformation_t *t, double const *lower_bounds,
    double const *upper_bounds, unsigned long len_of_bounds)
{
  unsigned i;
  double const *ub, *lb;
...
void boundary_transformation(boundary_transformation_t *t, double const *x, double *y,
    unsigned long len)
{
  double lb, ub, al, au;
  unsigned long i;
    ...
    double const *x, double *y, unsigned long len)
{
  ...
  for (i = 0; i < len; ++i)
  {
    ...
    {
      printf(
          "BUG in boundary_transformation_shift_into_feasible_preimage: lb=%f, ub=%f, al=%f au=%f, y=%f\n",
          lb, ub, al, au, y[i]);
      _FatalError("BUG");
    }
  }
}
void boundary_transformation_inverse(boundary_transformation_t *t, double const *x, double *y,
    unsigned long len)
{
  double lb, ub, al, au;
  unsigned long i;
